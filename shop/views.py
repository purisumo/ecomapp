...
def seller_dashboard_view(request):
    ...
    # Create a shop if not exists
    if not shop:
        base_name = f"Meowmin's Shop"
        name = base_name
        counter = 1

        # Ensure uniqueness of shop name
        while Shop.objects.filter(name=name).exists():
    ...
    total_revenue = OrderItem.objects.filter(
        order__shop=shop,
        order__status='completed'
    ).aggregate(total_revenue=Sum('subtotal'))['total_revenue'] or 0
...
def global_data(request):
    ...
    if request.user.is_authenticated:
    # if request.user.is_authenticated and hasattr(request.user, 'user_type') and request.user.user_type == 'customer':
        cart, _ = Cart.objects.get_or_create(customer=request.user)
        notifications = Notification.objects.filter(user=request.user).order_by('-created_at')
        shop = Shop.objects.filter(seller=request.user).first()
